...
class SimpleArticleImport
  ...
  def self.from_xml(directories,options={})
    ...
    logger=Logger.new("#{RAILS_ROOT}/log/simple_article_migration_#{Time.sr_now.to_date}.log")
    logger_article_id =Logger.new("#{RAILS_ROOT}/log/simple_article_new_id_#{Time.sr_now.to_date}.log")
    n=0
  ...
  if imgsrc =~ /((http|Http|https|Https):\/\/[a-z0-9_.-i\/]+)((\/images\d+|\/images\/)(.*(.jpg|.jpeg|.png|.gif|.JPG|.JPEG|.PNG|.GIF)))/
   image_full_path = data_path + $2 
    if File.exists?(image_full_path)
      ...
      `mv #{Rails.root}/#{anchor_tag.split('/').last} #{Rails.root}/NOKOGIRIMG`
      if File.exist?("#{Rails.root}/NOKOGIRIMG/#{anchor_tag.split('/').last}")
        asset_full_path = "#{Rails.root}/NOKOGIRIMG/#{anchor_tag.split('/').last}"
        return asset_full_path
      else
        puts "Not Exists!"
        return asset_full_path rescue nil
      end
    end
  return nil
  end
