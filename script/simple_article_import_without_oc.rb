...
class SimpleArticleImportWithoutOc
...
#debugger
      doc.find('channel').each do |node|
       ...
       old_id = (art_link - ["0","",".",".."]).last.split(".").first
           ref_xml_id = old_id if !old_id.blank?
           if old_id.blank?
        ...
        if !old_id.blank?
          ...
          ref_id =  file_path.split('/').last.scan(/\d+.xml/).to_s.to_i rescue "ciol"
            ...
            logger_article_id.info("ext_id --#{old_id}")
             find_id = XmlMigratedData.find(:first,:conditions=>["model_type=? and ext_id=? and publication_id=?","Article","#{ref_id}_#{ref_xml_id}",@xml_site_id.id])
            if find_id == nil
              article = Article.new()
...
#              article_content = doc.find(each_field.path+'/ArticleContent').first.content
           article_content = doc.find(each_field.path+'/articlecontent').first.content
             ...
             if !(article_content =~ /(<table)/).blank?
#debugger
       html_log = Logger.new("#{RAILS_ROOT}/log/html_tag_errors_#{options[:file_name]}_#{Time.now.to_date}.log")
        html_log.info("#{file_path}")
...
#debugger
             ...
             @xml =   XmlMigratedData.create(:model_type => "Article",:ext_id => "#{ref_id}_#{ref_xml_id}",:int_id => article.id,:publication_id => @xml_site_id.id,:old_url_part=>"",:previous_id=>"",:old_urls=>old_url)
                puts "article successfully save --->#{article.id}"
               puts "XmlMigratedData id #{@xml.ext_id}"
              logger_article_id.info("XmlMigratedData id : #{@xml.id} ---> #{@xml.ext_id}")
...
#             OpenCalaisByTag.article(@xml_site_id,new)
         begin
##	puts "Successfully created the redirect ==>#{old_url} to #{new.url}" if   puts "Successfully created the redirect ==>#{old_url} to #{new.url}" if ActiveRecord::Base.connection.execute("insert INTO all_redirects (host,orig_uri,destination,redirect_type,record_type,created_at,updated_at) VALUES ('#{@xml_site_id.name}','#{old_url}','#{new.url}','permanent','uri',now(),now())")
...
logger.info("created old url: #{old_url} and new url: #{new.url}   article id #{new.id}")
                ...
                logger.info("article not save --->#{file_path.split('/').last}")
                   ...
                   new = Article.find_by_id("#{find_id.int_id}") 
                    Ambient.init()
                  Ambient.current_site = @xml_site_id
            tag_er =    Logger.new("#{RAILS_ROOT}/log/HTML_TAG_ERRORS.log")

           tag_er.info("#{file_path.split("/").last} |-| #{find_id.int_id} |-| #{new.url}")
