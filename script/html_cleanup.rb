...
class HtmlCleanup
  def self.processing_xhtml(directories,options={})
    debugger
  logger = Logger.new("#{Rails.root}/log/#{options[:site_short_name]}_html_migration_#{Time.sr_now.to_date}.log")
    ...
    Dir["#{each_dir}/*.html"].each do |file_path |
     ...
     puts " -----1"
     debugger
    variable = IndesignCleanup.parasing_html(file_path,logger)

 if variable.size < 1
  log.info "\033[31mUsage:  #{variable.size}"
  STDERR.puts "Usage:  #{variable.size}".red
  STDERR.puts "Please check the variable size from html file #{file_path}".green
  log.info "\033[32mPlease check the variable size from html file #{file_path}"
  #exit 1
  html_doc= Nokogiri::HTML(variable, nil, 'UTF-8')
  dir= "/home/rameshs/Kreatio_Core/lib/data_migration"
  xslt  = Nokogiri::XSLT(File.read("#{dir}/cleanup.xslt"))
  debugger
  result_html = xslt.transform(html_doc).to_xhtml(:encoding => 'UTF-8')
  migration(file_path,result_html,options,logger)
end     
      end
    end
  end
  ...
  def self.migration(file_path,html,options,logger)
  doc = Nokogiri::HTML(html)
  old_id = file_path.split('/').last #.gsub(".html",'')
  ide_id = "#{options[:magazine_short_name]}.gsub(' ','-')_#{old_id}"  if options[:issue_volume] == true  
  ide_id = old_id if options[:issue_volume] == false
  Ambient.init
  site = Site.find(:first,:conditions=>['short_name=?',options[:site_short_name]])
  Ambient.current_site = site  
  find_id = XmlMigratedData.find(:first,:conditions=>["model_type=? and ext_id=? and publication_id=?","Article","#{ide_id}",site.id])
   if find_id.blank?
  # puts "Article Already there in DB"
   # else
    ...
    article.sites = [site]
    #Title
    article.title = file_path.split('/').last.gsub(/.(html|xhtml)/,'')
...
#   new_con = date.first.to_s.gsub("%20"," ").gsub("\n","").gsub("\t","").gsub("\r","").gsub(/ (class=\S+")/, "")
    ...
    article.image_id = image_ids if image_ids
...
=end
   ...
   article.publish_date = options[:issue_date] #Time.sr_now
...
debugger
      if article.save(:validate => false) 
        ...
        xmlmigratedata = XmlMigratedData.create(:model_type => "Article",:ext_id => "#{ide_id}",:int_id => article.id,:publication_id => site.id,:article_last_modify_date =>"#{Time.sr_now}",:old_url_part=>"",:previous_id=>"")  
