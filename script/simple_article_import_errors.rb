...
class SimpleArticleImport
  ...
  def self.from_xml(directories,options={})
    ...
    logger=Logger.new("#{RAILS_ROOT}/log/simple_article_migration_#{Time.now.to_s}.log")
    n=0
        ...
        if site_short_name[title] != nil
          ...
          doc.find(node.path+'/item').each do |each_field|
            ...
            if find_id == nil
              ...
              xml_tag_ids=[]
debugger     
             link =  doc.find(each_field.path+'/link').first.content.strip 
#              tag_name =  ((doc.find(each_field.path+'/articlelink').first.content.split('/') - ["0","",".",".."])[2]).gsub('-',' '){|name| name}.strip
                 tag_name = "" #(doc.find(each_field.path+'/articlelink').first.content.gsub("#{link}","") - ["0","",".",".."]).gsub("#{old_id}","").split('/').gsub('-',' '){|name| name}.strip
                tag_name =  ((doc.find(each_field.path+'/articlelink').first.content.gsub("#{}") 
...
log_inline =  Logger.new("#{RAILS_ROOT}/log/#{options[:site_short_name]}_inline_image_not_match.log") 
 if imgsrc
    data_path.each do|site_url,value|
      if imgsrc.match(/^#{Regexp.escape(site_url)}/)
      imgsrc=~/^#{Regexp.escape(site_url)}(.*)/i
log_inline.info("#{imgsrc} == #{site_url}  in XML #{file_path}")
      image_src_rest = $1
